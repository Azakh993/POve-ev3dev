...
{  
    ...
    tacho_set_position_sp(MOTOR_BOTH, 360);
    ...
    move_arm(1); //funktion som ska röra arm, ange minus ett för ena hållet. 
    move_arm(-1);
    fwd_m( 1 ); 
    rev_m( 1 ); 
    //funktion som ska röra arm, ange minus ett för ena hållet. 
    move_arm(1);
    move_arm(-1); //funktion som ska röra arm, ange minus ett för uppåt. 
...
void turn_L(){
    ...
    while ( gyro_ang !=-90 )
    {
        printf("%d",sensor_get_value(0, gyroSensor, 0 ));
...
void reset_gyro(){
    /*gyro_set_mode_gyro_g_and_a( GYRO );
    gyro_set_mode_gyro_ang( GYRO );*/
    sensor_set_mode(gyroSensor, LEGO_EV3_GYRO_GYRO_ANG);
    Sleep(500);
...
void find_wall(){
    ...
    temp_dist = sensor_get_value(US);
    shortest_distance = sensor_get_value(US)+1;
    ...
    while(do_loop){
        ...
        gyro_ang = sensor_get_value(0, gyroSensor, 0);
        tacho_run_forever( MOTOR_BOTH ); 
        temp_ang = sensor_get_value(US);

        if(temp_dist<shortest_distance){
            shortest_distance = sensor_get_value(US);
            correct_ang = sensor_get_value(0,gyroSensor,0);
        }
        if(temp_dist>shortest_distance){
            while(gyro_ang != correct_ang){
                //Turning left
                tacho_set_speed_sp( MOTOR_L, max_speed_L * -0.02);
                tacho_set_speed_sp( MOTOR_R, max_speed_L * 0.02);
...
void move_arm( int direction){
    tacho_set_speed_sp( MOTOR_C, max_speed_C * 0.1 * direction );
...
void tighten(){
    tacho_set_speed_sp( MOTOR_C, max_speed_C * 0.1 * -1);
    tacho_set_ramp_up_sp( MOTOR_C, max_rampUP_C * 0.01 );
    tacho_set_ramp_down_sp( MOTOR_C, max_rampDN_C * 0.01 );
